...
export default function LeaderboardPage() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from('.lb-badge', { opacity: 0, y: -12, duration: 0.4 })
        .from('.lb-title', { opacity: 0, y: 28, duration: 0.55 }, '-=0.2')
        .from('.lb-desc', { opacity: 0, y: 16, duration: 0.4 }, '-=0.2')
        .from('.lb-hero-ill', { opacity: 0, scale: 0.85, duration: 0.6, ease: 'back.out(1.4)' }, '-=0.3');
    }, headerRef);
    return () => ctx.revert();
  }, []);
    ...
    <div className="flex flex-col w-full bg-background min-h-screen text-foreground pb-24">
      ...
      <section ref={headerRef} className="border-b border-primary bg-background pt-8 pb-8">
        <div className="max-w-[1440px] mx-auto px-4 lg:px-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lb-hero-ill lg:col-span-5 flex justify-center lg:justify-end">
            <div className="w-full">
              <Img
                src="/leaderboard-hero.png"
                alt="Leaderboard Retro Illustration"
              />
            </div>
          </div>
        </div>
