...
export default function CreateBeastPage() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (!file.type.startsWith('image/')) {
      setUploadError('Invalid file format. Please upload an image file (PNG, JPG, WEBP, GIF).');
      return;
    }
    ...
    try {
      ...
      setUploadError(err?.message || 'Failed to upload image to ImgBB. Please check your network and try again.');
    } finally {
  ...
  const handleStatChange = useCallback((statKey: keyof BeastStats, delta: number) => {
    ...
    if (pointsBadgeRef.current) {
      gsap.fromTo(
        ...
        { scale: 1.25, backgroundColor: remainingPoints - delta === 0 ? 'var(--primary)' : 'var(--secondary)' },
  ...
  const handleForge = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !selectedAvatar || isSubmitting || remainingPoints > 0) return;
  ...
  return (
    <div className="flex flex-col w-full bg-background min-h-screen text-foreground pb-24">
      ...
      <section ref={headerRef} className="border-b border-primary bg-background pt-12 pb-8">
        <div className="max-w-[1440px] mx-auto px-4 lg:px-10">
          ...
          <p className="forge-desc font-sans text-sm sm:text-base text-secondary max-w-2xl leading-relaxed">
            Construct an apex synthetic combatant on Somnia Shannon. Upload your custom combatant avatar, allocate your 20-point attribute budget, configure tactical passives, and select your live DreamDEX market binding.
          </p>
        </div>
      </section>
      ...
      <section ref={formRef} className="max-w-[1440px] mx-auto w-full px-4 lg:px-10 pt-10">
        <form onSubmit={handleForge} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="forge-panel border border-primary p-6 bg-background space-y-4">
              <div className="flex items-center justify-between border-b border-primary pb-2">
                <span className="font-mono text-xs text-secondary uppercase font-bold">01 - CUSTOM AVATAR SYNTHESIS</span>
                <span className="font-mono text-xs text-primary font-bold">IMGBB STORAGE</span>
              </div>
            ...
            <div className="forge-panel border border-primary p-6 bg-background space-y-4">
              <div className="flex items-center justify-between border-b border-primary pb-2">
                <span className="font-mono text-xs text-secondary uppercase font-bold">02 - COMBAT IDENTIFIER</span>
                <span className="font-mono text-xs text-secondary">REQUIRED</span>
              </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="forge-panel border border-primary p-6 bg-background space-y-6">
              <div className="flex items-center justify-between border-b border-primary pb-3">
                <div>
                  <span className="font-mono text-xs text-secondary uppercase font-bold">03 - ATTRIBUTE MATRIX</span>
                  <h2 className="font-headline font-bold text-2xl uppercase tracking-tight text-primary">
                    POINT ALLOCATION
                  </h2>
                </div>
                <div className="text-right">
                  <span className="font-mono text-[10px] text-secondary block uppercase">POINTS REMAINING</span>
                  <span 
                    ref={pointsBadgeRef}
              ...
              <div className="space-y-4">
                ...
                <div className="border border-neutral p-4 bg-surface-container-low flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 self-end sm:self-auto">
                    ...
                    <span 
                      ref={(el) => { statRefs.current['power'] = el; }}
                    ...
                    <button
                      type="button"
                      onClick={() => handleStatChange('power', 1)}
                      disabled={stats.power >= STAT_BUDGET.MAX_PER_STAT || remainingPoints <= 0}
                ...
                <div className="border border-neutral p-4 bg-surface-container-low flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="font-mono text-xs text-secondary">
                      Reduces incoming kinetic & energy damage from opponent turns.
                    </div>
                  </div>
                  <div className="flex items-center gap-3 self-end sm:self-auto">
                    <button
                    ...
                    <span 
                      ref={(el) => { statRefs.current['defense'] = el; }}
                      className="font-headline font-extrabold text-2xl w-8 text-center text-primary"
                    >
                      {stats.defense}
                    </span>
                    <button
                      type="button"
                      onClick={() => handleStatChange('defense', 1)}
                      disabled={stats.defense >= STAT_BUDGET.MAX_PER_STAT || remainingPoints <= 0}
                      className="w-9 h-9 bg-primary text-background border border-primary font-headline font-bold text-lg hover:bg-secondary disabled:opacity-30 disabled:hover:bg-primary transition-colors flex items-center justify-center"
                    >
                      +
                    </button>
                ...
                <div className="border border-neutral p-4 bg-surface-container-low flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 self-end sm:self-auto">
                    ...
                    <button
                      type="button"
                      onClick={() => handleStatChange('speed', 1)}
                      disabled={stats.speed >= STAT_BUDGET.MAX_PER_STAT || remainingPoints <= 0}
                      className="w-9 h-9 bg-primary text-background border border-primary font-headline font-bold text-lg hover:bg-secondary disabled:opacity-30 disabled:hover:bg-primary transition-colors flex items-center justify-center"
                    >
                      +
                    </button>
                ...
                <div className="border border-neutral p-4 bg-surface-container-low flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 self-end sm:self-auto">
                    ...
                    <span 
                      ref={(el) => { statRefs.current['special'] = el; }}
                      className="font-headline font-extrabold text-2xl w-8 text-center text-primary"
                    >
                      {stats.special}
                    </span>
                    <button
                      type="button"
                      onClick={() => handleStatChange('special', 1)}
                      disabled={stats.special >= STAT_BUDGET.MAX_PER_STAT || remainingPoints <= 0}
                      className="w-9 h-9 bg-primary text-background border border-primary font-headline font-bold text-lg hover:bg-secondary disabled:opacity-30 disabled:hover:bg-primary transition-colors flex items-center justify-center"
                    >
                      +
                    </button>
            ...
            <div className="forge-panel border border-primary p-6 bg-background space-y-4">
              <div className="flex items-center justify-between border-b border-primary pb-2">
                <div>
                  <span className="font-mono text-xs text-secondary uppercase font-bold">04 - TACTICAL PASSIVES</span>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        ...
                        {isSelected && <FiCheck className="w-4 h-4 text-background flex-shrink-0" />}
                      </div>
                      <p className={`font-sans text-xs leading-relaxed ${isSelected ? 'text-background/80' : 'text-secondary'}`}>
                        {perk.description}
                      </p>
                      <span className={`font-mono text-[10px] font-bold ${isSelected ? 'text-background' : 'text-primary'}`}>
                        {perk.effectSummary}
                      </span>
                    </button>
                  );
            ...
            <div className="forge-panel border border-primary p-6 bg-background space-y-4">
              <div className="flex items-center justify-between border-b border-primary pb-2">
                <div>
                  <span className="font-mono text-xs text-secondary uppercase font-bold">05 - DREAMDEX MARKET BINDING</span>
                  <div className="font-headline font-bold text-lg uppercase text-primary">
                    EVENT CONTRACT TELEMETRY
                  </div>
                </div>
                <span className="font-mono text-xs text-secondary">OPTIONAL</span>
              </div>

              <p className="font-sans text-xs text-secondary leading-relaxed">
                Bind your combatant to a real-time crypto price prediction market. Live order book Up/Down probability will grant dynamic in-battle stat modifiers.
              </p>
              ...
              <div className="grid grid-cols-3 gap-3 pt-1">
                {(['BTC', 'ETH', 'UNBOUND'] as BoundAsset[]).map((asset) => {
                  ...
                  return (
                    ...
                    >
                      {asset === 'UNBOUND' ? 'UNBOUND (NONE)' : `${asset} BOUND`}
                    </button>
                  );
                })}
              </div>
            ...
            <button
              ref={submitBtnRef}
              type="submit"
              disabled={isSubmitting || remainingPoints > 0 || !name.trim() || !selectedAvatar}
